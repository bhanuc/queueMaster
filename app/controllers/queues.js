...
const tarantool = require(`${appRoot}/lib/tarantool`);
...
const Queues = {
	...
	createOneQueue: async (ctx) => {
		...
		const {queueType} = ctx.request.body;
		// const {options} = ctx.request.body;
		const conn = tarantool.getConnection();
		const results = await conn.eval(`return require("queue").create_tube('${queueName}', '${queueType}')`);
		ctx.body = results;
	},
	putTask: async (ctx) => {
		const {queueName} = ctx.request.body;
